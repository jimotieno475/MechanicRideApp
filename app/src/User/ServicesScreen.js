...
import { View, Text, ScrollView, TouchableOpacity, Alert, Animated, TextInput, ActivityIndicator } from 'react-native';
...
export default function ServicesScreen() {
  ...
  const onSelectService = (service) => {
    Animated.sequence([
      ...
      navigation.navigate('UserMapScreen', { service });
    });
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-black">
      <View className="px-5 pt-6 pb-3 border-b border-gray-700">
        <Text className="text-3xl font-extrabold text-white">Car Services</Text>
        <Text className="text-gray-400 mt-1 text-base">Select the service you need</Text>
        ...
        <View className="flex-row flex-wrap justify-between">
          {filteredServices.map(service => {
            ...
            return (
              <View
                key={service.id}
                className={`w-[48%] rounded-xl mb-5 border ${isSelected ? 'border-white' : 'border-transparent'}`}
                style={{
              ...
              >
                ...
                >
                  <Ionicons name="car-sport-outline" size={28} color={isSelected ? '#000' : '#ddd'} />
                </View>

                <Text className={`text-center mb-1 text-lg font-semibold ${isSelected ? 'text-white' : 'text-gray-300'}`}>
                  {service.name}
                </Text>
                <Text className="text-center mb-4 text-gray-400 text-sm">{service.description || 'No description'}</Text>

                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => onSelectService(service)}
                  className={`mx-8 mb-4 rounded-lg py-2 items-center justify-center ${isSelected ? 'bg-white' : 'bg-gray-800'}`}
                >
                  <Animated.Text
                    style={{
