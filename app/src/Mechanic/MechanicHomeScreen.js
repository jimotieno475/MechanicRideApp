...
export default function MechanicHomeScreen() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const bookingsWithRatings = await Promise.all(
        data.map(async (booking) => {
          if (booking.status === 'Completed') {
            try {
              const ratingRes = await fetch(`${API_URL}/bookings/${booking.id}/rating`);
              if (ratingRes.ok) {
                const ratingData = await ratingRes.json();
                return { ...booking, rating: ratingData.rating };
              }
            } catch (error) {
              console.error("Error fetching rating for booking:", booking.id, error);
            }
          }
      ...
      setRequests(bookingsWithRatings);
    } catch (err) {
    ...
    if (mechanic) {
      if (!socketRef.current) {
         const socket = io(WS_URL, {
          ...
          socket.on("BOOKING_UPDATED", async (updatedBooking) => {
            console.log("🔄 BOOKING_UPDATED received:", updatedBooking);
            
            // If booking is completed, fetch the rating
            if (updatedBooking.status === 'Completed') {
              try {
                const ratingRes = await fetch(`${API_URL}/bookings/${updatedBooking.id}/rating`);
                if (ratingRes.ok) {
                  const ratingData = await ratingRes.json();
                  updatedBooking.rating = ratingData.rating;
                }
              } catch (error) {
                console.error("Error fetching rating for updated booking:", updatedBooking.id, error);
              }
            }
      ...
      <View className="bg-white rounded-xl p-4 mb-4 shadow-md">
        ...
        {item.status === 'Completed' && (
          <View className="mt-2 p-2 bg-gray-50 rounded-lg">
            <Text className="text-sm font-semibold text-gray-700">Customer Rating:</Text>
            {item.rating ? (
              <View className="mt-1">
              ...
              <Text className="text-gray-500 text-xs mt-1">No rating yet</Text>
            )}
          </View>
        )}
